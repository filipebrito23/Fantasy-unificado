...
def close_expired_bids_v5():
    ...
    with engine.begin() as conn:
        ...
        for row in rows:
            conn.execute(
                text("UPDATE player_state SET status = 'CLOSED' WHERE player_id = :player_id"),
...
def submit_bid_v5(player_id, team_id, amount, years, username, user_id, role, forced_team_id=None):
    ...
    with engine.begin() as conn:
        player = conn.execute(text("""
            SELECT player_id, owner_team_id, status, COALESCE(is_active, 1) AS is_active
            FROM players
            WHERE player_id = :player_id
        ...
        if current_active and current_active['active_bid_id']:
            conn.execute(
                text("UPDATE bids SET is_active = 0 WHERE bid_id = :bid_id"),
                {"bid_id": current_active['active_bid_id']}
            )

        bid_id = conn.execute(text("""
            INSERT INTO bids (
                player_id, team_id, amount, years, created_at,
                is_valid, is_active, is_renewal, created_by_user_id
            )
            VALUES (
                :player_id, :team_id, :amount, :years, :created_at,
                1, 1, :is_renewal, :created_by_user_id
            )
            RETURNING bid_id
        """), {
            'player_id': player_id,
        ...
        exists = conn.execute(
            text("SELECT COUNT(*) FROM player_state WHERE player_id = :player_id"),
            {"player_id": player_id}
        ).scalar()
        ...
        if exists:
            ...
            conn.execute(text("""
                INSERT INTO player_state (
                    player_id, active_bid_id, active_team_id, active_amount,
                    active_years, active_at, expires_at, status, is_renewal
                )
                VALUES (
                    :player_id, :bid_id, :team_id, :amount,
                    :years, :active_at, :expires_at, 'OPEN', :is_renewal
                )
            """), payload)
...
def update_bid_v5(bid_id, amount, years, username, user_id):
    with engine.begin() as conn:
        row = conn.execute(text("""
            SELECT bid_id, player_id, team_id, is_active, deleted_at
            FROM bids
            WHERE bid_id = :bid_id
        """), {"bid_id": bid_id}).mappings().first()
...
def delete_bid_v5(bid_id, username, user_id, reason=None):
    ...
    with engine.begin() as conn:
        row = conn.execute(text("""
            SELECT bid_id, is_active, deleted_at
            FROM bids
            WHERE bid_id = :bid_id
        """), {"bid_id": bid_id}).mappings().first()
...
def get_players_with_state_v5(position=None):
    query = """
    SELECT p.player_id, p.player_name, p.position, owner.team_name AS dono,
           t.team_name AS time_ativo, ps.active_amount AS proposta_ativa,
           ps.active_years AS anos, ps.active_at, ps.expires_at,
...
def get_bid_history_v5(player_id):
    with engine.begin() as conn:
        return conn.execute(text("""
            SELECT b.bid_id, t.team_name, b.amount, b.years, b.created_at,
                   b.updated_at, b.deleted_at, b.delete_reason,
                   b.is_active, b.is_renewal, u.email AS created_by
            FROM bids b
...
def get_all_bids_v5(limit=200):
    with engine.begin() as conn:
        return conn.execute(text("""
            SELECT b.bid_id, p.player_name, p.position, t.team_name, b.amount,
                   b.years, b.created_at, b.updated_at, b.deleted_at,
                   b.delete_reason, b.is_active, b.is_valid, b.is_renewal,

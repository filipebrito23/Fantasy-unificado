...
def ensure_default_home_tabs():
    ...
    with engine.begin() as conn:
        for row in defaults:
            conn.execute(text("""
                INSERT INTO home_tabs (tab_key, tab_label, is_active, sort_order, allow_posts, allow_comments)
                SELECT :tab_key, :tab_label, :is_active, :sort_order, :allow_posts, :allow_comments
                WHERE NOT EXISTS (
...
def get_draft_board():
    with engine.begin() as conn:
        result = conn.execute(text("""
            SELECT draft_id, season, round, pick_number, original_team_id, current_team_id,
...
def get_posts_by_tab(tab_key: str):
    with engine.begin() as conn:
        result = conn.execute(text("""
            SELECT post_id, tab_key, author, title, content_md, status, is_pinned, created_at, updated_at
            FROM home_posts
...
def create_post(tab_key: str, author: str, title: str, content_md: str, is_pinned: bool = False):
    with engine.begin() as conn:
        conn.execute(text("""
            INSERT INTO home_posts (tab_key, author, title, content_md, status, is_pinned)
            VALUES (:tab_key, :author, :title, :content_md, 'PUBLISHED', :is_pinned)
        """), {
            "tab_key": tab_key,
            "author": author,
            "title": title,
...
def get_comments(tab_key: str, post_id=None):
    sql = """
        SELECT comment_id, tab_key, post_id, author, comment_text, related_pick_id, status, created_at, is_pinned
        FROM wall_comments
        WHERE lower(tab_key) = lower(:tab_key)
...
def create_comment(tab_key: str, author: str, comment_text: str, post_id=None, related_pick_id=None, is_pinned: bool = False):
    with engine.begin() as conn:
        conn.execute(text("""
            INSERT INTO wall_comments (tab_key, post_id, author, comment_text, related_pick_id, status, is_pinned)
            VALUES (:tab_key, :post_id, :author, :comment_text, :related_pick_id, 'VISIBLE', :is_pinned)
        """), {
            "tab_key": tab_key,
            "post_id": post_id,
            "author": author,

...
def get_team_cap_status_v5(conn, team_id):
    cap_limit = conn.execute(
        text(
            "SELECT cap_limit FROM teams WHERE team_id = :team_id"
        ),
        {"team_id": team_id},
    ).scalar()
...
def close_expired_bids_v5():
    ...
    with engine.begin() as conn:
        ...
        for row in rows:
            conn.execute(
                text(
                    "UPDATE player_state SET status = 'CLOSED' WHERE player_id = :player_id"
                ),
...
def _contract_year_limits(amount: float):
    """
    Retorna (min_anos, max_anos) conforme faixa salarial.
    Se amount < 1.000.000, retorna None (valor mínimo).
    """
    amount = float(amount)
...
def _validate_contract_years(amount: float, years: int):
    """
    Valida anos conforme faixa salarial.
    Retorna None se OK, ou string de erro.
    """
    limits = _contract_year_limits(amount)

    if limits is None:
        return "O valor mínimo da proposta é R$ 1.000.000,00."

    min_years, max_years = limits

    if years < min_years or years > max_years:
        if min_years == max_years:
            return (
                f"Para uma proposta de R$ {amount:,.2f}, "
                f"a duração deve ser de {min_years} ano(s)."
            )

        return (
            f"Para uma proposta de R$ {amount:,.2f}, "
            f"a duração deve ser de {min_years} a {max_years} anos."
        )
...
def submit_bid_v5(
    ...
    years_error = _validate_contract_years(amount, years)
    if years_error:
        return False, years_error
    ...
    with engine.begin() as conn:
        ...
        if player["is_active"] != 1:
            return False, "Jogador inativo na base atual."
        ...
        if current_active:
            if current_active["active_amount"] is not None:
                current_amount = float(current_active["active_amount"])

            if current_active["active_years"] is not None:
                current_years = int(current_active["active_years"])

        if current_amount is not None:
            if is_renewal and amount < current_amount:
                return (
                    False,
                    "Proposta de renovação não pode ser menor que "
                    f"a proposta ativa ({current_amount:,.2f}).",
                )

            if not is_renewal and amount <= current_amount:
                return (
                    False,
                    "Nova proposta deve ser maior que "
                    f"a proposta ativa ({current_amount:,.2f}).",
                )

            if (
                is_renewal
                and abs(amount - current_amount) < 0.01
                and current_years is not None
                and years != current_years
            ):
                return (
                    False,
                    "Quando a renovação cobre uma proposta de mesmo valor, "
                    f"a duração deve ser de {current_years} anos.",
                )

        effective_available = available

        if not is_renewal:
            effective_available += released_amount

        if not is_renewal and amount > effective_available:
            return (
                False,
                f"Cap insuficiente. Disponível: {effective_available:,.2f}",
            )

...
with bid_tab:
    ...
    if open_players.empty:
        ...
        st.write(
            ...
            f"{formatar_brl_v5(float(selected_row['proposta_ativa'])) if pd.notna(selected_row['proposta_ativa']) else '-'}"
        )
        ...
        st.caption(f"Por extenso: {valor_por_extenso_v5(amount)}")
    ...
    if user["role"] != "admin":
        ...
        if not bids_df.empty:
            ...
            selected_bid_id = st.selectbox(
                "Selecionar proposta",
                bid_options,
                format_func=lambda bid_id: (
                    f"#{bid_id} - "
                    f"{bids_df.loc[bids_df['bid_id'] == bid_id, 'player_name'].iloc[0]} / "
                    f"{bids_df.loc[bids_df['bid_id'] == bid_id, 'team_name'].iloc[0]}"
                ),
            )
            ...
            with st.form("admin_edit_bid_form"):
                new_amount = st.number_input(
                    "Novo valor",
                    value=float(selected_bid["amount"]),
                    min_value=1000000.0,
                    step=100000.0,
                    format="%.2f",
                )
...
with profile_tab:
    ...
    with st.form("change_password_profile_v5"):
        ...
        confirm_password = st.text_input(
            ...
            type="password",
            key="profile_confirm_password_v5",
        )

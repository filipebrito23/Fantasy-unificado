...
- Cada aba tem:
  - Linha 1: Nome do Time (ocupando varias colunas)
  - Linha 2: Headers (Pos, Jogadores, GameID, Pontes, Rebotes, etc.)
...
# Mapeamento de colunas (nomes exatos dos headers nas planilhas)
COLUMN_MAP = {
...
def get_team_sigla_from_aba(aba_name: str, team_position: str) -> str:
    """Extrai a sigla do time a partir do nome da aba.
...
def get_team_id(team_sigla: str, conn) -> int | None:
    """Busca o team_id com base na sigla do time."""
    result = conn.execute(
        text(
            """
            SELECT team_id
            FROM teams
            WHERE sigla = :team_sigla
            LIMIT 1
            """
        ),
        {"team_sigla": team_sigla},
    )
    row = result.fetchone()
    return row[0] if row else None
...
def parse_team_data(values: list, start_row: int, col_indices: dict, conn, team_id: int) -> list[dict]:
    """Parse dos dados de um time (6 jogadores).
    
    start_row: indice da primeira linha de jogadores (0-based)
    """
    players = []
    ...
    for row_idx in range(start_row, start_row + 6):
        ...
        # Nome do jogador está na coluna "Jogadores"
...
def import_games_from_google_sheets():
    ...
    # Contador global de jogos para source_game_id unico
    ...
    with engine.begin() as conn:
        ...
        for url_info in urls_data:
            ...
            # Contador de jogos dentro desta planilha
            game_in_sheet_counter = 0
            ...
            for sheet in sheets:
                ...
                # Extrair siglas dos times da aba
                ...
                # Buscar team_ids pelas siglas
                team_a_id = get_team_id(team_a_sigla, conn)
                team_b_id = get_team_id(team_b_sigla, conn)
                
                if team_a_id is None or team_b_id is None:
                    print(f"    Times nao encontrados no banco. Pulando.\n")
                    continue
                
                # Ler dados da aba (linhas 1-20)
                ...
                # ========================================
                # Time A: headers na linha 2 (indice 1), jogadores nas linhas 3-8 (indices 2-7)
                # ========================================
                headers_a = values[1] if len(values) > 1 else []
                
                # Mapear indices das colunas (case-insensitive)
                col_indices = {}
                ...
                if not team_a_players:
                    print(f"    Nenhum jogador encontrado para Time A. Pulando.\n")
                    continue
                
                # ========================================
                # Time B: headers na linha 12 (indice 11), jogadores nas linhas 13-18 (indices 12-17)
                # ========================================
                headers_b = values[11] if len(values) > 11 else []
                
                # Mapear indices das colunas para Time B
                col_indices_b = {}
                ...
                if not team_b_players:
                    print(f"    Nenhum jogador encontrado para Time B. Pulando.\n")
                    continue
                ...
                # Gerar source_game_id unico: round * 1000 + contador_do_jogo
                game_in_sheet_counter += 1
                source_game_id = round_num * 1000 + game_in_sheet_counter
                
                # Verificar se jogo ja existe (usando round + team_1_id + team_2_id)
                ...
                if existing_game:
                    ...
                    # Inserir novo jogo com source_game_id unico
                    result = conn.execute(
                        text(
                            """
                        ...
                        ),
                        {
                            ...
                            "team_1_id": team_a_id,
                            "team_1_name": team_a_sigla,
                            "team_1_points": team_1_points,
                            "team_2_points": team_2_points,
                            "team_2_id": team_b_id,
                            "team_2_name": team_b_sigla,
                            "round": round_num,
